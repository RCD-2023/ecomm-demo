...
import { insertCartSchema } from '../validator';
import { revalidatePath } from 'next/cache';
// import { Prisma } from '@prisma/client';


// Calculate cart total price based on items
...
export const addItemToCart = async (data: z.infer<typeof cartItemSchema>) => {
  try {
    // Check if the current user (browser/session) has a cart ID assigned
...
await prisma.cart.update({
  where: { id: cart.id },
  data: {
    items: cart.items, //aici era items: cart.items as Prisma.CartUpdateitemsInput[],
